...
if(isset($_REQUEST['ptsql']) && isset($_REQUEST['pagina'])){
    ...
    if ($s=="salariati.S1='Salariatii ATB'"){ //$_REQUEST['ptsql']=="S1='Salariatii ATB'"
        ...
        if (strlen($s)==0){
             $sql="SELECT salariati.Cheie, salariati.Marca, salariati.NPren, salariati.CNP, salariati.DNastere, salariati.Sex, salariati.Func, salariati.SitCIM, urgente.Ambulanta, urgente.Data, urgente.Ora, urgente.Observatii FROM (salariati JOIN urgente ON urgente.Marca=salariati.Marca) ORDER BY salariati.NPren";  
        }else{
        $s=substr($_REQUEST['ptsql'],34,strlen($_REQUEST['ptsql']));
        $sql="SELECT salariati.Cheie, salariati.Marca, salariati.NPren, salariati.CNP, salariati.DNastere, salariati.Sex, salariati.Func, salariati.SitCIM, urgente.Ambulanta, urgente.Data, urgente.Ora, urgente.Observatii  FROM (salariati JOIN urgente ON urgente.Marca=salariati.Marca) WHERE ".$s." ORDER BY salariati.NPren";         
        }
    }else{
        $sql="SELECT salariati.Cheie, salariati.Marca, salariati.NPren, salariati.CNP, salariati.DNastere, salariati.Sex, salariati.Func, salariati.SitCIM, urgente.Ambulanta, urgente.Data, urgente.Ora, urgente.Observatii FROM (salariati JOIN urgente ON urgente.Marca=salariati.Marca) WHERE ".$_REQUEST['ptsql']." ORDER BY salariati.NPren";  
